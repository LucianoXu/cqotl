...
and tactic2str (t: tactic) : string =
    ...
    | Sorry               -> 
        "sorry."
    | Expand x            ->
        Printf.sprintf "expand %s." x
    | Refl                -> 
        "refl."
    | Destruct v          -> 
        Printf.sprintf "destruct %s." v
    | Case e              -> 
        Printf.sprintf "case %s." (term2str e)
    | Intro v             -> 
        Printf.sprintf "intro %s." v
    | Revert v            -> 
        Printf.sprintf "revert %s." v
    | Apply e             -> 
        Printf.sprintf "apply %s." (term2str e)
    | Choose i            -> 
    ...
    | Simpl               -> 
        "simpl."
    | Rewrite_L2R e       -> 
        Printf.sprintf "rewrite %s." (term2str e)
    | Rewrite_R2L e       -> 
        Printf.sprintf "rewrite <    - %s." (term2str e)
    | RWRULE r            -> 
        Printf.sprintf "rwrule %s." (rwrule2str r)

    | R_PRE e             -> 
        Printf.sprintf "r_pre %s." (term2str e)
    | R_POST e            -> 
        Printf.sprintf "r_post %s." (term2str e)
    | R_SKIP              -> 
        "r_skip."
    | R_SEQ (n1, n2, t)   -> 
    ...
    | R_MEAS              -> 
        "r_meas."
    | R_IF qs             -> 
        Printf.sprintf "r_if %s." (term2str qs)
    | R_WHILE (qs, phi)   -> 
        Printf.sprintf "r_while %s %s." (term2str qs) (term2str phi)
    | R_WHILE_WHILE (qs, phi) -> 
        Printf.sprintf "r_while_while %s %s." (term2str qs) (term2str phi)
    | R_MEAS_MEAS switch  -> 
    ...
    | CYLINDER_EXT e      -> 
        Printf.sprintf "cylinder_ext %s." (term2str e)
...
and term2str (e: terms) : string =
    ...
    | Fun {head; args=[t1; t2]} when head = _zeroo && t1 = t2   ->
        Printf.sprintf "0O[%s]" (term2str t1)
